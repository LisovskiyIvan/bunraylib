import { Raylib, COLORS, CURSOR } from "../src";
...
while (!Raylib.windowShouldClose()) {
  ...
  const lineStripPoints = new Float32Array([270, 40, 290, 20, 310, 40, 330, 60, 500, 10]);
  Raylib.drawLineStrip(lineStripPoints, lineStripPoints.length, COLORS.CYAN);
  Raylib.drawLineBezier({ x: 340, y: 60 }, { x: 390, y: 20 }, 2, COLORS.MAGENTA);
  ...
  Raylib.drawRectanglePro({ x: 610, y: 280, width: 150, height: 60 }, { x: 75, y: 30 }, Math.PI / 6, COLORS.YELLOW);
  ...
  Raylib.drawRectangleGradientEx({ x: 440, y: 360, width: 150, height: 50 }, COLORS.RED, COLORS.GREEN, COLORS.BLUE, COLORS.YELLOW);
  Raylib.drawRectangleLines(100, 430, 150, 50, COLORS.WHITE);
  Raylib.drawRectangleLinesEx({ x: 270, y: 430, width: 150, height: 50 }, 3, COLORS.CYAN);
  Raylib.drawRectangleRounded({ x: 440, y: 430, width: 150, height: 50 }, 5, 16, COLORS.MAGENTA);
  Raylib.drawRectangleRoundedLinesEx({ x: 100, y: 500, width: 150, height: 50 }, 2, 16, 2, COLORS.WHITE);

  Raylib.drawTriangle({ x: 350, y: 500 }, { x: 300, y: 560 }, { x: 400, y: 560 }, COLORS.RED);
  Raylib.drawTriangleLines({ x: 450, y: 500 }, { x: 400, y: 560 }, { x: 500, y: 560 }, COLORS.GREEN);

  const fan = new Float32Array([600, 530, 550, 580, 575, 550, 600, 580, 625, 550, 650, 580]);
  Raylib.drawTriangleFan(fan, fan.length, COLORS.BLUE);

  const strip = new Float32Array([680, 500, 700, 560, 720, 520, 740, 580]);
  Raylib.drawTriangleStrip(strip, strip.length, COLORS.YELLOW);
  ...
  const sp = new Float32Array([50, 700, 200, 650, 350, 750, 500, 680, 650, 720, 800, 660, 950, 700]);
  Raylib.drawSplineLinear(sp, sp.length, 2, COLORS.WHITE);
  Raylib.drawSplineCatmullRom(sp, sp.length, 2, COLORS.CYAN);
  Raylib.drawSplineBezierQuadratic(sp, sp.length, 3, COLORS.MAGENTA);
